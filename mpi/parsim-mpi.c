...
void create_mpi_particle() {
    ...
    MPI_Datatype types[9] = {MPI_LONG_LONG, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_INT};
    ...
    MPI_Get_address(&temp_particle.collided, &displacements[8]);

    for (int i = 0; i < 9; i++) {
        displacements[i] -= base_address;
    }

    MPI_Type_create_struct(9, block_lengths, displacements, types, &MPI_particle_t);
...
particle_t copy_particle(particle_t *particle) {
    ...
    //copy.collided = particle->collided;
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < ncside; j++) {
            ...
            for (int p1 = 0; p1 < cell->index; p1++) {
                for (int p2 = p1 + 1; p2 < cell->index; p2++) {
                ...
                particle_t *particle_to_remove = &cell->particles[p1];
                if (particle_to_remove->collided == 1)
                {
...
int main(int argc, char **argv)
{
...

    {
        ...
        printf("Process %d: %ld collisions\n", id, total_num_collisions);
    }
    ...
    for (long i = 0; i < ROW_SIZE(id,p,ncside); i++) {
        for (long j = 0; j < ncside; j++) {
            for (long idx = 0; idx < grid[i][j].index; idx++) {
                if (grid[i][j].particles[idx].id == 1) {
