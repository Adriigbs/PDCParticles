...
void create_mpi_particle() {
    ...
    MPI_Datatype types[8] = {MPI_CHAR, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE};
                      ...
                      double side, int id, int p, char *has_main_particle)
{
    ...
    for (long y = 0; y < n_rows; y++) {
        for (long x = 0; x < ncside; x++) {
            ...
            while (i < cell->index) {
                ...
                if (new_row >= process_low && new_row < process_high) { // moved to the same process
                    remove_particle_from_cell(cell, i);
                    add_particle_to_buffer(&to_move, &to_move_count, &size_to_move, copy);
                } else if (new_row == ROW_HIGH(above, p, ncside)-1) { // moved to the above process
                    if (copy.id == 1) {
                        *has_main_particle = 0;
                    }
                    remove_particle_from_cell(cell, i);
                    add_particle_to_buffer(&above_particles_to_send, &above_particles_count, &size_above, copy);
                } else if (new_row == ROW_LOW(below, p, ncside)) { // moved to the below process
                    if (copy.id == 1) {
                        *has_main_particle = 0;
                    }
                    remove_particle_from_cell(cell, i);
    ...
    MPI_Status count_status[2];
    MPI_Request count_send_requests[2]; // dois sets de dois requests, para nao...
    MPI_Request count_recv_requests[2]; // dar overwrite em requests

    MPI_Isend(&above_particles_count, 1, MPI_LONG, above, 1, MPI_COMM_WORLD, &count_send_requests[0]);
    MPI_Isend(&below_particles_count, 1, MPI_LONG, below, 1, MPI_COMM_WORLD, &count_send_requests[1]);

    MPI_Irecv(&recv_above_particles_count, 1, MPI_LONG, above, 1, MPI_COMM_WORLD, &count_recv_requests[0]);
    MPI_Irecv(&recv_below_particles_count, 1, MPI_LONG, below, 1, MPI_COMM_WORLD, &count_recv_requests[1]);

    MPI_Waitall(2, count_recv_requests, count_status);
...
int main(int argc, char **argv)
{
    ...
    if (has_main_particle == 0) MPI_Wait(&request, &status);
