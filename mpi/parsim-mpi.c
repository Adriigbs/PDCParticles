...
int main(int argc, char **argv)
{
    ...
    long long local_n_part = n_part / p;

    // Allocate memory for particles 
    // Initialize particles
    particle_t *particles = (particle_t*) malloc(n_part * sizeof(particle_t));
    particle_t *local_particles = (particle_t*) malloc(local_n_part * sizeof(particle_t));
    ...
    MPI_Scatter(particles, local_n_part * sizeof(particle_t), MPI_BYTE,
                local_particles, local_n_part * sizeof(particle_t), MPI_BYTE,
                0, MPI_COMM_WORLD);
    ...
    for (long i = 0; i < time_steps; i++) {
        ...
        calculate_center_of_mass(particles, n_part, ncside, grid, cell_side, seed);
        long local_collisions = detect_collisions(local_particles, &local_n_part, ncside, grid);
        MPI_Reduce(&local_collisions, &total_num_collisions, 1, MPI_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
    }

    MPI_Gather(local_particles, local_n_part * sizeof(particle_t), MPI_BYTE,
               particles, local_n_part * sizeof(particle_t), MPI_BYTE,
               0, MPI_COMM_WORLD);
